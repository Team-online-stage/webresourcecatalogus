...
class CheckinFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $template->setContent('Beste {{ receiver.givenName }},<p>Welkom bij checkin!</p><p>Uw verzoek tot deelname van checkin met referentie {{ resource.reference }} is met succes ingediend.</p>{% set organization = commonground_resource(resource.properties[\'horeca_onderneming_contact\']) %}<p>De door u opgegeven gegevens van uw organisatie: <br>Naam: {{organization.name}}<br>Kvk: {% if organization.kvk is defined and organization.kvk is not empty %}{{organization.kvk}}{% else %}{{resource.properties[\'kvk\']}}{% endif %}</p>{% set nodes =  commonground_resource_list({\'component\': \'chin\', \'type\': \'nodes\'},{\'organization\':organization[\'@id\']})[\'hydra:member\'] %}{% if nodes|length > 0 %}{% set node = nodes[0] %}{% endif %}<p>Hierbij hebben wij alvast een voorbeeld QR-code voor u: <br>{{node.reference}}</p><p>Algemene voorwaarden.</p><p>Verwerkings overeenkomst.</p><p>Met vriendelijke groet,</p>{{ sender.name }}');
        ...
        $template->setContent('Beste {{ receiver.givenName }},<p><p>U kunt nu inloggen op https://dev.checking.nu/ met de volgende inlognaam: {% if receiver.emails|length >0 %}{% set receiverEmail = receiver.emails[0] %}{% endif %}{% if receiverEmail is defined and receiverEmail is not empty %}{{ receiverEmail.email }}{% endif %}</p><p>Met vriendelijke groet,</p>{{ sender.name }}');
        $template->setTemplateEngine('twig');
        $manager->persist($template);
        $template->setId($id);
        $manager->persist($template);
